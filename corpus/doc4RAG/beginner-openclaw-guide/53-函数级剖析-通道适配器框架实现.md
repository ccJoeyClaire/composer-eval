...
1. manifest 声明：`extensions/<your-channel>/openclaw.plugin.json`
2. 插件注册：`extensions/<your-channel>/src/index.ts` -> `api.registerChannel({ plugin })`
3. 注册入库：`src/plugins/registry.ts` -> `registerChannel(...)` -> `registry.channels.push(...)`
4. 网关启动：`src/gateway/server.impl.ts` -> `createChannelManager(...)`
5. 通道启动：`src/gateway/server-channels.ts` -> `startChannels()` / `startChannel(...)`
6. 入站路由：`src/routing/resolve-route.ts` -> `resolveAgentRoute(...)`
7. 出站发送：`src/infra/outbound/deliver.ts` -> `deliverOutboundPayloads(...)`
8. 状态体检：`src/gateway/server-methods/channels.ts` -> `"channels.status"`
...
先有 `openclaw.plugin.json`。它告诉 OpenClaw：这个插件的 ID、能力、配置 schema、启用提示是什么。

这一步能让 Gateway 在不执行插件业务代码的情况下，先完成发现、诊断、配置校验和启用规划。
